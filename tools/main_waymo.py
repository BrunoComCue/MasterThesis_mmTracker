...
parser.add_argument('--det_name', type=str, default='vel_det_results')
parser.add_argument('--process', type=int, default=1)
parser.add_argument('--visualize', action='store_true', default=False)
parser.add_argument('--start_frame', type=int, default=0, help='start at a middle frame for debug')
parser.add_argument('--obj_type', type=str, default='pedestrian', choices=['vehicle', 'pedestrian', 'cyclist'])
# paths
parser.add_argument('--config_path', type=str, default='/home/bruno/Desktop/thesis_repo/SimpleTrack/configs/waymo_configs/vc_both_giou.yaml', help='config file path, follow the path in the documentation')
...
parser.add_argument('--gt_folder', type=str, default='/media/bruno/toshibaHDD/data_folder/detection/gt/dets/')
...
def gt_bbox2world(bboxes, egos):
    ...
    for i in range(frame_num):
        ...
        for j in range(bbox_num):                                  # at this point the bbox are still on local coordinate frame
...
def sequence_mot(configs, data_loader: WaymoLoader, sequence_id, gt_bboxes=None, gt_ids=None, visualize=False):
    ...
    for frame_index in range(data_loader.cur_frame, frame_num):    #it goes through each frame
...
def main(name, obj_type, config_path, data_folder, det_data_folder, result_folder, gt_folder, start_frame=0, token=0, process=1):
    ...
    if obj_type == 'vehicle':
        ...
        type_token = 4
    for file_index, file_name in enumerate(file_names[:]):   #### it goes through each sequence
        ...
        gt_bboxes, gt_ids = load_gt_bboxes(gt_folder, data_folder, segment_name, type_token)    # at this point the bbox are still on local coordinate frame
