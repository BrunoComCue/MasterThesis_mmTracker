...
class HitManager:
    def __init__(self, configs, frame_index):
        ...
        self.trk_del_threshold = configs['running']['trk_deletion_threshold']                  ###
...
#            self.recent_state = 1
    ...
    def update(self, update_info: UpdateInfoData, trk_score, is_key_frame=True):
        ...
        if is_key_frame:
            ...
            self.state_transition(trk_score)        
...
#                self.state = 'dead'
    ...
    def state_transition(self, trk_score):                                                          ###
       #if (trk_score <= self.trk_del_threshold and self.time_since_update >= self.max_age):          ###   a tracklet is kill only under 2 conditions: score and age
       if (trk_score <= self.trk_del_threshold):
           self.state = 'dead'
