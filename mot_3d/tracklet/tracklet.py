...
class Tracklet:
    def __init__(self, configs, id, bbox: BBox, det_type, frame_index, time_stamp=None, aux_info=None):
        ...
        self.trk_score = bbox.s                              ###  the initial score of the tracklet is the one of the associated/matched detection                  
    ...
    def predict(self, time_stamp=None, is_key_frame=True):
        ...
        self.trk_score = self.trk_score * 0.01                                  ###
        #self.trk_score -=self.score_decay                                        ###
        result.s = self.trk_score
        return result
    ...
    def update(self, update_info: UpdateInfoData):
        ...
        if update_info.mode == 1 or update_info.mode == 3:
            ...
            #self.trk_score -= self.score_decay                                        ### score decay only for unmatched tracklets
            self.trk_score = 1 - ((1 - self.trk_score)*(1 - update_info.bbox.s))       ###
        else:
            self.trk_score -= self.score_decay                                         ###only score decay for unmatched tracklets
        #self.life_manager.update(update_info, is_key_frame)                           ###
        self.life_manager.update(update_info, self.trk_score)
        return
...
#        return self.life_manager.valid_output(frame_index)
    def valid_output(self):
        return self.life_manager.valid_output()                              ###
