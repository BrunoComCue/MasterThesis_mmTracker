...
class Tracklet:
    ...
    def weight(self, kf_model, cv_model):
        ...
        if 0 <= d < 3000:
            ...
            result.x = 0.5 * cv_model.x + 0.5 * result.x
            result.y = 0.5 * cv_model.y + 0.5 * result.y
            result.z = 0.5 * cv_model.z + 0.5 * result.z
        else:
            result.x = 0.1 * cv_model.x + 0.7 * result.x
            result.y = 0.1 * cv_model.y + 0.7 * result.y
            result.z = 0.1 * cv_model.z + 0.7 * result.z
        return result
