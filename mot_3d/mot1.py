...
class MOTModel:
    ...
    def frame_mot(self, input_data: FrameData):
        ...
        # for when it is not a key frame (and it ends here the frame_mot() function) -> for nuScenes
        if not input_data.aux_info['is_key_frame']:
            result = self.non_key_frame_mot(input_data)
            return result

        #matched, unmatched_dets, unmatched_trks = self.forward_step_trk(input_data)   #KF motion model
        matched, unmatched_dets, unmatched_trks = self.back_step_det(input_data)    #CV motion model
            ...
            if t not in unmatched_trks:
                for k in range(len(matched)):
                    if matched[k][1] == t:
                        d = matched[k][0]
                        break
                aux_info = {                                                       #CV motion model
                    'velo': list(input_data.aux_info['velos'][d]),
                    'is_key_frame': input_data.aux_info['is_key_frame']}
#                aux_info = {'is_key_frame': input_data.aux_info['is_key_frame']}    #KF motion model
                ...
                result_bbox, update_mode = self.redundancy.infer(trk, input_data, time_lag)
                aux_info = {                                                       #CV motion model
                    'velo': np.zeros(2),
                    'is_key_frame': input_data.aux_info['is_key_frame']}
...
#            aux_info = {'is_key_frame': input_data.aux_info['is_key_frame']}  # KF motion model
    ...
    def back_step_det(self, input_data: FrameData):
        ...
        # association
        matched, unmatched_dets, unmatched_trks = associate_dets_to_tracks(det_preds, trk_preds,
