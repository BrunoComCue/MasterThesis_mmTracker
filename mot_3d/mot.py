    ...
    def frame_mot(self, input_data: FrameData):
        ...
        for t, trk in enumerate(self.trackers):
            if t not in unmatched_trks:
                ...
                trk.update(update_info)
#                if trk.death():                                      ### check for dead tracklets just for the unmatched ones (the matched ones are obviously not dead)
#                    self.trackers.pop(t)
        ...
        for trk in self.trackers:
            result.append((trk.get_state(), trk.id, trk.state_string(self.frame_count), trk.det_type))
