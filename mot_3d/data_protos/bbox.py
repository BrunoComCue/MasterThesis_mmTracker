...
class BBox:
    ...
    def box_pts2world(cls, ego_matrix, pcs):
        new_pcs = np.concatenate((pcs,                                        #### pcs is the center / corner coordinates
                                  np.ones(pcs.shape[0])[:, np.newaxis]),
                                  axis=1)
        new_pcs = ego_matrix @ new_pcs.T                                        #### this line transforms the coordinates from local to global (matrix multiplication)
        new_pcs = new_pcs.T[:, :3]
        return new_pcs
    ...
    @classmethod
    def bbox2world(cls, ego_matrix, box):     # this changes the coordinate frame from local to world
        # center and corners
...
#    @classmethod
#    def center2local(cls, ego_matrix, center):
#        old_center = np.concatenate((center,       # center coordinates in world frame
#                                  np.ones(center.shape[0])[:, np.newaxis]),
#                                 axis=1)
#        new_center = np.linalg.inv(ego_matrix) @ old_center.T   # this line transforms the coordinates from world to local (matrix multiplication)
#        new_center = new_center.T[:, :3]
#        return new_center
    ...
    def center2local(cls, ego_matrix, center):
        old_center = np.concatenate((center,       # center coordinates in world frame
                                  np.ones(center.shape[0])[:, np.newaxis]),
                                 axis=1)
        return (np.linalg.inv(ego_matrix) @ old_center.T).T[:, :3]